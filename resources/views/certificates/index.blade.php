...
<div class="page-heading">
    <div>
        <h1>Certificates</h1>
        <p>Search, verify, and manage all report certificates.</p>
    </div>
    @canMutate
            ...
            <tbody>
                ...
                @forelse($certificates as $certificate)
                    <tr>
                        ...
                        <td>{{ $certificate->location ?: 'N/A' }}</td>
                        ...
                        <td>{{ $certificate->report_validity_date ? \Carbon\Carbon::parse($certificate->report_validity_date)->format('d-m-Y') : 'N/A' }}</td>
                ...
                @empty
                    <tr><td colspan="12" class="text-center text-muted py-4">No certificates found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            $.get(@json(route('liveSearch')), { userInput: query }, function (response) {
                var rows = response.data.data.map(function (item, index) {
                    ...
                    return '<tr><td><input class="form-check-input certificate-select" type="checkbox" value="' +
                        ...
                        '<td>' + escapeHtml(item.report_prepared_by) + '</td>' +
                        '<td>' + escapeHtml(item.report_approved_by) + '</td>' +
                        '<td>' + formatDate(item.report_issue_date) + '</td>' +
                        '<td>' + formatDate(item.report_validity_date) + '</td>' +
                ...
                $('.search-result tbody').html(rows || '<tr><td colspan="12" class="text-center py-4">No matching certificates.</td></tr>');
                syncSelectionUi();
            });
        }, 250);
    });
