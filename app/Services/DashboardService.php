...
class DashboardService
{
    ...
    private function monthlyReports(): array
    {
        ...
        $counts = Certificate::whereNotNull('report_issue_date')
            ->where('report_issue_date', '>=', $start)
            ->get(['report_issue_date'])
            ->groupBy(function ($certificate) {
                try {
                    return Carbon::parse($certificate->report_issue_date)->format('Y-m');
    ...
    {
        if (!Schema::hasTable('reports_activity_logs')) {
            return collect();
        }
